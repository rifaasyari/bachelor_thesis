...
for SNR = 0:snr_step:SNR_max
    ...
    for i = 1:runs  % simulation runs
        H = randn(N_r, N_t) + 1j * randn(N_r, N_t); 
        ...
        for k = 1:transmitted_symbols
            ...
            if mblrsic_dec == true
                s_mblrsic = mb_lr_sic(y,H,var_s,N,modulation,constellation,true,true);
                b_mblrsic = decode(s_mblrsic, constellation);
                BE_mblrsic = BE_mblrsic + sum(b ~= b_mblrsic);
            end

            if lr_sic_dec == true
                s_lrsic = lr_sic(y,H,var_s,N,modulation,constellation,true,true);
                b_lrsic = decode(s_lrsic, constellation);
                BE_lrsic = BE_lrsic + sum(b ~= b_lrsic);
            end

            if mbsic_dec == true
                s_mbsic = mb_lr_sic(y,H,var_s,N,modulation,constellation,false,false);
                b_mbsic = decode(s_mbsic, constellation);
                BE_mbsic = BE_mbsic + sum(b ~= b_mbsic);
            end

            if sic_dec == true
                s_sic = lr_sic(y,H,var_s,N,modulation,constellation,false,false);
...
if lr_sic_dec == true
    semilogy(0:snr_step:SNR_max, BER_lrsic, 'b-+', ... 
        'LineWidth', 2, ...
        'DisplayName', 'LR-SIC');
    hold on
end
